...
def main() -> None:
    ...
    p_odds.add_argument("--atr-stops", action="store_true",
                        help="size each simulated trade's risk off the REAL rolling "
                             "ATR(14) at entry (1.5x, clamped 0.3%%-5%%), matching the "
                             "live funded desk's atr_stops=True, instead of a fixed "
                             "stop_loss_pct for every trade (session 47)")
    p_odds.add_argument("--symbols", default="",
                        help="comma-separated symbols for --real (default: the live "
                             "desk's actual --funded watchlist, not just 5 FX majors)")
